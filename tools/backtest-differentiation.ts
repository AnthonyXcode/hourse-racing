 ...
 *
 * MC agreement gate (applied after the MC run):
 *   --mc-min=75     skip unless the pick's MC Place% >= 75
 *
 * Trip experience (past runs in the chosen --form source):
 *   --min-trip-runs=3   skip unless the pick has run the exact race distance >= 3 times
 *
...
async function main() {
  const { sparseMax, closeMax, avgDiffMin, gapMin, oddsMax, ratingChangeMin, months, venue, surface, ignoreClasses, ignoreDistances, maxRating, maxAvgDiff, mcMin, minTripRuns, form, ignoreAfter } =
  ...
  console.log(
    `  8. Trip experience: top-rated past runs at race distance < ${minTripRuns && minTripRuns > 0 ? minTripRuns : "disabled"} (--min-trip-runs) → skip`
